...
async function createItinerary(req, res, next) {
    try {
        ...
        const existing = await prisma_1.prisma.itinerary.findFirst({ where: { slug, deleted_at: null } });
        if (existing) {
            slug = `${slug}-${Date.now()}`;
        }
        ...
        const daysData = body.days_list || [];
        const featuresData = body.features || [];
        const inclusionsData = body.inclusions || [];
        const exclusionsData = body.exclusions || [];
        const imagesData = body.images || [];
        const item = await prisma_1.prisma.itinerary.create({
            data: {
                title: body.title,
                slug,
...
async function updateItinerary(req, res, next) {
    try {
        const { id } = req.params;
        const body = req.body;
        const existing = await prisma_1.prisma.itinerary.findFirst({ where: { id, deleted_at: null } });
        if (!existing) {
            return res.status(404).json({ success: false, message: "Itinerary not found." });
        }
        ...
        if (body.slug && body.slug !== existing.slug) {
            ...
            const duplicate = await prisma_1.prisma.itinerary.findFirst({
        ...
        if (body.destination_ids) {
            await prisma_1.prisma.itineraryDestination.deleteMany({ where: { itinerary_id: id } });
        }
        if (body.days_list) {
            await prisma_1.prisma.itineraryDay.deleteMany({ where: { itinerary_id: id } });
        }
        if (body.features) {
            await prisma_1.prisma.itineraryFeature.deleteMany({ where: { itinerary_id: id } });
        }
        if (body.inclusions) {
            await prisma_1.prisma.itineraryInclusion.deleteMany({ where: { itinerary_id: id } });
        }
        if (body.exclusions) {
            await prisma_1.prisma.itineraryExclusion.deleteMany({ where: { itinerary_id: id } });
        }
        if (body.images) {
            await prisma_1.prisma.itineraryImage.deleteMany({ where: { itinerary_id: id } });
        }
        const updated = await prisma_1.prisma.itinerary.update({
            where: { id },
            data: {
                ...
                destinations: body.destination_ids
                    ? {
                        create: body.destination_ids.map((destId, idx) => ({
                            destination_id: destId,
                            display_order: idx,
                        })),
                    }
                ...
                days_list: body.days_list
                    ? {
                        create: body.days_list.map((d, idx) => ({
                            day_number: d.day_number || idx + 1,
                            title: d.title,
                            description: d.description,
                            image_url: d.image_url,
                ...
                features: body.features
                    ? {
                        create: body.features.map((f, idx) => ({
                            name: typeof f === "string" ? f : f.name,
                            icon: f.icon || null,
                            display_order: idx,
                        })),
                ...
                inclusions: body.inclusions
                    ? {
                        create: body.inclusions.map((inc, idx) => ({
                            title: typeof inc === "string" ? inc : inc.title,
                            display_order: idx,
                        })),
                    }
                ...
                exclusions: body.exclusions
                    ? {
                        create: body.exclusions.map((exc, idx) => ({
                            title: typeof exc === "string" ? exc : exc.title,
                            display_order: idx,
                        })),
                    }
