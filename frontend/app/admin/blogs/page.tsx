...
export default function AdminBlogsListPage() {
  ...
  const handleStatusToggle = async (id: string, currentStatus: string) => {
    ...
    try {
      const res = await fetch(`/api/v1/admin/blogs/${id}/status`, {
        method: "PATCH",
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-2xl border border-gray-200 shadow-xs flex gap-4 justify-between items-center">
        <form onSubmit={(e) => { e.preventDefault(); setPage(1); fetchBlogs(); }} className="relative flex-1">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-xs overflow-hidden">
        {loading ? (
          <div className="py-20 text-center text-xs text-gray-500">Loading blog posts...</div>
        ) : items.length === 0 ? (
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-gray-50/80 transition-colors">
                      ...
                      <td className="py-3 px-4 text-gray-700">
                        {item.author_name || "The Luxe Yatra"}
                      </td>
                      <td className="py-3 px-4">
                        {item.is_featured ? (
                      ...
                      <td className="py-3 px-4 text-right space-x-2">
                        ...
                        <Link href={`/admin/blogs/${item.id}/edit`} className="p-1.5 rounded-lg border text-gray-600 hover:bg-gray-100 inline-block">
