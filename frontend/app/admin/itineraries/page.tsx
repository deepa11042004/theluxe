...
import { Plus, Search, Edit, Trash2, CheckCircle2, XCircle, Star, Calendar } from "lucide-react";
import StatusBadge from "@/components/admin/StatusBadge";

export default function AdminItinerariesListPage() {
  ...
  const fetchItineraries = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/v1/admin/itineraries?${params.toString()}`);
  ...
  const handleStatusToggle = async (id: string, currentStatus: string) => {
    ...
    try {
      const res = await fetch(`/api/v1/admin/itineraries/${id}/status`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ status: nextStatus }),
      });
      const json = await res.json();
      if (json.success) fetchItineraries();
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/v1/admin/itineraries/${id}`, { method: "DELETE" });
      const json = await res.json();
      if (json.success) fetchItineraries();
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-2xl border border-gray-200 shadow-xs flex gap-4 justify-between items-center">
        <form onSubmit={(e) => { e.preventDefault(); setPage(1); fetchItineraries(); }} className="relative flex-1">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-xs overflow-hidden">
        {loading ? (
          <div className="py-20 text-center text-xs text-gray-500">Loading itineraries...</div>
        ) : items.length === 0 ? (
          <div className="py-20 text-center text-xs text-gray-400">No itineraries found.</div>
        ) : (
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-gray-50/80 transition-colors">
                      ...
                      <td className="py-3 px-4 font-semibold text-gray-900">
                        {item.price_from ? `${item.price_currency || "INR"} ${item.price_from.toLocaleString()}` : "On Request"}
                      </td>
                      <td className="py-3 px-4">
                        {item.is_featured ? (
                      ...
                      <td className="py-3 px-4 text-right space-x-2">
                        <button onClick={() => handleStatusToggle(item.id, item.status)} className="p-1.5 rounded-lg border text-gray-600 hover:bg-gray-100">
                          {item.status === "PUBLISHED" ? <XCircle className="w-4 h-4 text-amber-600" /> : <CheckCircle2 className="w-4 h-4 text-emerald-600" />}
                        </button>
                        <Link href={`/admin/itineraries/${item.id}/edit`} className="p-1.5 rounded-lg border text-gray-600 hover:bg-gray-100 inline-block">
